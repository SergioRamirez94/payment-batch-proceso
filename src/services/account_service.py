...
import logging
import numpy as np
from utils.sqs_notification import send_notification
...
def suggest_name(user_name:str, db_session:Session, tenant: Tenant):
    try:
        suggested_name = user_name
        counter = 1
        while True and counter < 1000:
            counter += 1
            account = db_session.query(Account).filter(Account.user_name == suggested_name, Account.tenant == tenant.value).first()
            if account is None:
                return suggest_name
            random_number = random.randint(100, 999)
            suggested_name = f"{user_name}{random_number}"
        return None
...
def create_account_name(db_session:Session, tenant: Tenant):
    ...
    suggested_name = suggest_name(user_name, db_session, tenant)
    if suggested_name is None:
        raise ValueError("Error suggesting account name.")
    return suggested_name


def create_accounts_batch(db_session:Session, df: pd.DataFrame, currency:Currency, tenant:Tenant, s3_key):
    ...
    df_account_to_create['user_name'] = df_account_to_create.apply(lambda x: create_account_name(tenant), axis=1)
